...
static uint32_t s_learn_ttl  = REPEATER_FDB_DEFAULT_TTL_S;
static bool     s_dhcp_snoop = true;

void repeater_set_learn_ttl(uint32_t ttl_seconds) {
    s_learn_ttl = ttl_seconds ? ttl_seconds : REPEATER_FDB_DEFAULT_TTL_S;
}
...
static void snoop_dhcp_client(uint8_t *eth, size_t len) {
    if (!s_dhcp_snoop) return;
    size_t dlen = 0;
    const uint8_t *dhcp_ro = locate_dhcp(eth, len, 68, 67, &dlen);
    if (!dhcp_ro) return;
    dhcp_parsed_t info;
    if (!dhcp_parse(dhcp_ro, dlen, &info)) return;
    if (info.op != DHCP_BOOTP_REQUEST) return;

    /* Force the BOOTP BROADCAST flag so the server replies to ff:ff:ff:ff:ff:ff.
     * Required for WiFi L2 bridging: a unicast OFFER/ACK addressed to the real
     * client MAC would never reach our STA — the upstream AP has no association
     * for that MAC, and the STA's radio MAC filter accepts only the STA's own
     * MAC plus group addresses. */
    uint8_t *dhcp = eth + (dhcp_ro - eth);
    uint8_t *udp_hdr = dhcp - 8;
    dhcp_force_broadcast(dhcp, dlen, udp_hdr);
...
static void snoop_dhcp_server_reply(const uint8_t *eth, size_t len, uint8_t chaddr_out[6]) {
    ...
    ESP_LOGD(TAG, "DHCP server xid=0x%08" PRIx32 " type=%u yiaddr=0x%08" PRIx32,
             info.xid, info.msg_type, info.yiaddr);
    if (info.msg_type == DHCPACK && info.yiaddr != 0) {
        /* DHCP-ACK supplies a lease — prefer it over the data-plane fallback. */
        uint32_t ttl = info.lease_time ? info.lease_time : s_learn_ttl;
        fdb_learn(info.yiaddr, info.chaddr, ttl);
...
static bool bridge_ap_to_sta(struct pbuf *p) {
    ...
    if (etype == ETYPE_ARP) {
        ...
        uint32_t tpa;
        memcpy(&tpa, arp + 24, 4);
        uint32_t sta_ip = netif_ip4_addr(s_sta_netif)->addr;
    ...
    if (etype == ETYPE_IP4) {
        ...
        if (iphdr->src.addr != 0) {
            fdb_learn(iphdr->src.addr, eth + 6, s_learn_ttl);
        }
        uint32_t ap_ip  = s_ap_netif  ? netif_ip4_addr(s_ap_netif)->addr  : 0;
        uint32_t sta_ip = s_sta_netif ? netif_ip4_addr(s_sta_netif)->addr : 0;
