...
/* Set the BOOTP BROADCAST flag (high bit of the flags field at offset 10) on
 * a client→server DHCP packet so the server replies via L2 broadcast. Needed
 * for L2 WiFi bridging: the upstream AP won't deliver unicast 802.11 frames
 * to a client MAC that isn't associated with it, and our STA's hardware MAC
 * filter only accepts the STA MAC and group addresses. RFC 1542 §5.4 / RFC
 * 2131 §4.1 explicitly authorize relay agents to set this bit.
 *
 * Updates the UDP checksum incrementally (RFC 1624). If the original UDP
 * checksum was 0 (legal for IPv4), it is left at 0.
 *
 *   buf:     mutable BOOTP/DHCP payload, length >= DHCP_MIN_LEN (240).
 *   len:     length of the BOOTP payload.
 *   udp_hdr: mutable pointer to the 8-byte UDP header preceding buf, or NULL
 *            to skip checksum maintenance.
 *
 * Returns true if the flag was newly set, false if it was already set or
